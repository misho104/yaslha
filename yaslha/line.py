import enum
import re
from typing import Any, List, Optional, Pattern, Union, cast  # noqa: F401

import yaslha.exceptions
from yaslha.utility import ChannelType, KeyType, ValueType, _float
...
class BlockLine(AbsLine):
    ...
    def __init__(
        self, name: str, q: Optional[StrFloat] = None, comment: str = ""
    ) -> None:
...
class DecayBlockLine(AbsLine):
    ...
    def __str__(self):
        # type: ()->str
        return "DECAY {:>9}   {:16.8e}   # {}".format(
            self.pid, self.width, self.comment.lstrip
        ).rstrip()
...
class InfoLine(AbsLine):
    ...
    def append(
        self, value: Union[str, List[str]], comment: Union[str, List[str]] = ""
    ) -> None:
...
class TwoIndexLine(ValueLine):
    ...
    def __init__(
        self, i1: StrInt, i2: StrInt, value: ValueType, comment: str = ""
    ) -> None:
...
class ThreeIndexLine(ValueLine):
    ...
    def __init__(
        self, i1: int, i2: int, i3: int, value: ValueType, comment: str = ""
    ) -> None:
...
class DecayLine(ValueLine):
    ...
    IN = (
        ...
        + cap(INT, "nda")
        + SEP
        + cap(r"[0-9\s+-]+", "daughters")
        + TAIL
    )

    def __init__(
        self, br, nda=0, daughters="", channel=None, comment=""
    ):  # nda is not used.
...
def parse_string_in_info_block(line: str) -> Optional[AbsLine]:
    for cls in [CommentLine, BlockLine, DecayBlockLine, InfoLine]:
        obj = cast(AbsLine, cls).construct(line)
        if obj:
            return obj
    return None
