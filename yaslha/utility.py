...
def _clean(obj):
    ...
    if isinstance(obj, OrderedDict):
        ...
        return list(
            v for v in obj if not (v is None or (hasattr(v, "__len__") and len(v) == 0))
        )
    else:
        return obj
...
def sort_pids_default(pids: List[U]) -> List[Union[U, int]]:
    """Sort block names according to specified order."""
    tmp = dict(
        (key, []) for key in PID_GROUPS
    )  # type: MutableMapping[str, List[Union[U, int]]]
...
def copy_sorted_decay_block(decay, sort_by_br=True):
    ...
    def ordering(pid):
        ...
        return (
            ...
            abs(pid),
            pid < 0,
        )  # SUSY first, smaller first, positive first
    ...
    if sort_by_br:
        ...
        while i < len(tmp):
            ...
            i = j
        ch_mapping = _flatten(
            [sorted(x, key=lambda m: sort_key(m[1])) for x in tmp2], level=1
        )
