...
from yaslha.utility import KeyType, _clean, _flatten, _float  # noqa: F401
...
class AbsDumper:
    def __init__(self, blocks_order=None, values_order=None, comments_preserve=None):
        ...
        self.comments_preserve = config.value(
            "comments_preserve", comments_preserve, typ=CommentsPreserve
        )
    ...
    def _block_lines_with_key_order(self, block):
        ...
        value_lines = block.value_lines(
            with_comment_lines=self.comments_preserve.keep_line()
        )

        keys = list(value_lines.keys())
        if self.values_order == ValuesOrder.DEFAULT and block.name == "MASS":
            keys_ = yaslha.utility.sort_pids_default(
                keys
            )  # to suppress strange mypy complaint
            keys = keys_
    ...
    def _decay_lines_with_key_order(self, decay):
        ...
        if self.values_order == ValuesOrder.DEFAULT:
            sorted_decay = yaslha.utility.copy_sorted_decay_block(
                decay, sort_by_br=True
            )
        elif self.values_order == ValuesOrder.SORTED:
            sorted_decay = yaslha.utility.copy_sorted_decay_block(
                decay, sort_by_br=False
            )
        else:
            sorted_decay = decay
        lines = _flatten(
            list(
                sorted_decay.value_lines(
                    with_comment_lines=self.comments_preserve.keep_line()
                ).values()
            )
        )
...
class SLHADumper(AbsDumper):
    ...
    def __init__(
        self,
        separate_blocks=None,
        forbid_last_linebreak=None,
        ...
        **kwargs
        ...
        self.separate_blocks = config.value(
            "separate_blocks", separate_blocks, typ=bool
        )
        self.forbid_last_linebreak = config.value(
            "forbid_last_linebreak", forbid_last_linebreak, typ=bool
        )
        self.document_blocks = [
            name.upper() if isinstance(name, str) else name
            for name in config.value("document_blocks", document_blocks)
    ...
    def dump(self, slha):
        # type: (yaslha.SLHA)->str
        blocks = [
            self.dump_block(block, document_block=(block.name in self.document_blocks))
            for block in self._blocks_sorted(slha)
        ]
        decays = [
            self.dump_decay(decay, document_block=(decay.pid in self.document_blocks))
        ...
        if self.write_version:
            lines.insert(
                0, "# written by {} {}".format(yaslha.__pkgname__, yaslha.__version__)
            )
    ...
    def dump_block(self, block, document_block=False):
        ...
        if self.comments_preserve.keep_line():
            pre_comment = cast(
                List[yaslha.line.AbsLine], block.line_comment(CommentPosition.Prefix)
            )
            head_comment = cast(
                List[yaslha.line.AbsLine], block.line_comment(CommentPosition.Heading)
            )
            head = pre_comment + head + head_comment
            tail = cast(
                List[yaslha.line.AbsLine], block.line_comment(CommentPosition.Suffix)
            )

        lines_raw = _clean(
            _flatten(
                [
                    self.dump_line(obj, block_name=block.name)
                    for obj in head + body + tail
                ]
            )
        )
    ...
    def dump_value_line(self, obj, block_name):
        ...
        if block_name == "MASS" and isinstance(obj.key, int):
            return " {:>9}   {}   # {}".format(
                obj.key, self._e_float(obj.value), obj.comment.lstrip()
            ).rstrip()
        ...
        return " {}   {}   # {}".format(
            key_str, value_str, obj.comment.lstrip()
        ).rstrip()
    ...
    def dump_decay(self, decay, document_block=False):
        ...
        if self.comments_preserve.keep_line():
            pre_comment = cast(
                List[yaslha.line.AbsLine], decay.line_comment(CommentPosition.Prefix)
            )
            head_comment = cast(
                List[yaslha.line.AbsLine], decay.line_comment(CommentPosition.Heading)
            )
            head = pre_comment + head + head_comment
            tail = cast(
                List[yaslha.line.AbsLine], decay.line_comment(CommentPosition.Suffix)
            )

        lines = cast(
            List[str], _clean([self.dump_line(obj) for obj in head + body + tail])
        )
    ...
    def dump_decayblock_line(self, obj):
        ...
        return "{} {:>9}   {}   # {}".format(
            self.decay_str, obj.pid, self._e_float(obj.width), obj.comment.lstrip()
        ).rstrip()
    ...
    def dump_decay_line(self, obj):
        ...
        return "   {}   {:>2}   {}  # {}".format(
            self._e_float(obj.value), len(obj.key), ids_str, obj.comment.lstrip()
        ).rstrip()
...
class AbsMarshalDumper(AbsDumper):
            ...
            OrderedDict(
                [
                        ...
                        OrderedDict(
                            [
                                ...
                                (
                                    "FORMATTER",
                                    "{} {}".format(
                                        yaslha.__pkgname__, yaslha.__version__
                                    ),
                                ),
                    ...
                    (
                        ...
                        OrderedDict(
                            [
                                (b.name, self.marshal_block(b))
                                for b in self._blocks_sorted(slha)
                            ]
                        ),
                    ),
                    (
                        ...
                        OrderedDict(
                            [
                                (d.pid, self.marshal_decay(d))
                                for d in self._decays_sorted(slha)
                            ]
                        ),
                    ),
                    (
    ...
    def marshal_block(self, block: "yaslha.Block") -> Mapping[Any, Any]:
        data = OrderedDict(
            [("info", None), ("values", None), ("comments", list())]
        )  # type: MutableMapping[str, Any]
        values, key_order = self._block_lines_with_key_order(block)
        values_without_comment_lines = _flatten(
            [v for v in values if not isinstance(v, yaslha.line.CommentLine)]
        )
        if block.q:
            data["info"] = ["Q=", block.q]
        if self.comments_preserve.keep_line():
            ...
            for c_pos in CommentPosition:
                if block.line_comment(c_pos):
                    data["comments"].append(
                        [c_pos.name, [v.line for v in block.line_comment(c_pos)]]
                    )
        ...
        data["values"] = list(
            [self.marshal_line(line) for line in values_without_comment_lines]
        )
    ...
    def marshal_decay(self, decay):
        ...
        data = OrderedDict(
            [("info", [decay.width]), ("values", None), ("comments", list())]
        )  # type: MutableMapping[str, Any]
        ...
        if self.comments_preserve.keep_line():
            ...
            for c_pos in yaslha.line.CommentPosition:
                if decay.line_comment(c_pos):
                    data["comments"].append(
                        [c_pos.name, [v.line for v in decay.line_comment(c_pos)]]
                    )
            # comments between values
